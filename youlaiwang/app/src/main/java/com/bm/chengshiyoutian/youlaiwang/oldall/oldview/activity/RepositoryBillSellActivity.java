...
public class RepositoryBillSellActivity extends Activity implements View.OnClickListener, View.OnTouchListener {
    ...
    private void initView() {
        if ("1".equals(selectType)) {
            title.setText(getString(R.string.repositorybillsell_one));
    ...
    private void initData() {
        ...
        } else if ("2".equals(selectType)) {  //如果 selectType 为 1 这个页面的明细列表，根据“生产-仓库台账（入库）”页面；
            getGoodsClass();
        }
    }
    ...
    private void selectPhotoDialog() {
        ...
        takePhoto.setOnClickListener(new View.OnClickListener() {
            ...
            @Override
            public void onClick(View v) {
                img.goToCamera(RepositoryBillSellActivity.this);//通过拍照获取图片
                popupWindow.dismiss();
            }
        });
